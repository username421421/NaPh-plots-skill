...
These are all the objects we need for our simulation! Let's resolve the constraints and plot the simulation scene to see if we made any mistakes. Note that it is good practice to split the random key to maintain randomness in JAX (see <a href = "https://docs.jax.dev/en/latest/notebooks/thinking_in_jax.html#pseudorandom-numbers">here</a> for more details)
